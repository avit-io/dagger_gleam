...
// =============================================================================
// 3. RENDERING PHANTOM TYPES + OPT TYPE E ENCODE_OPTS
// =============================================================================

fn render_phantom_decls(decls: List(#(String, List(String)))) -> String {
  case decls {
    [] -> ""
    _ ->
      decls
      |> list.map(fn(d) {
        let #(type_name, variants) = d
        case variants {
          [] -> "pub type " <> type_name <> "\n"
          _ ->
            "pub type "
...
fn render_none(opts: List(OptDecl)) -> String {
  case opts {
    [] -> ""
    _ -> "pub fn none(opts: Opts(a)) -> Opts(a) {\n  opts\n}\n\n"
  }
}

fn render_setters(opts: List(OptDecl)) -> String {
  case opts {
    [] -> ""
    _ ->
      opts
...
fn render_function_encoder(
  ...
  case relevant_variants {
    ...
    _ -> {
      ...
      "fn encode_"
      <> fn_full_name
      <> "(opts: Opts(tag_)) -> List(#(String, types.Value)) {\n"
      <> "  list.filter_map([\n"
      <> cases
      <> "\n  ], fn(x) { x })\n"
      <> "}\n\n"
...
fn generate_grouped_module(
  ...
  // Merge phantom_decls da tutti i tipi del gruppo, deduplicando per type_name
  let merged_phantom_decls =
    list.flat_map(type_data, fn(td) {
      let #(_, m) = td
      m.phantom_decls
    })
    |> list.fold([], fn(acc, d) {
      case list.any(acc, fn(x: #(String, List(String))) { x.0 == d.0 }) {
        True -> acc
        False -> list.append(acc, [d])
      }
    })
  let imports = module_imports(terminal, merged_opts != [])
  let opt_section =
    render_phantom_decls(merged_phantom_decls)
    <> render_opts_record(merged_opts)
    <> render_defaults(merged_opts)
    <> render_none(merged_opts)
    <> render_setters(merged_opts)
