...
1. **The Tree (Data)** — Using generated modules like `container` or `host`, you describe your pipeline. This stage is pure, side-effect free, and fast. You are just building a data structure.
2. **The Interpreter (Execution)** — When you call a terminal function like `stdout`, the interpreter scans your tree, resolves any deferred dependencies (e.g. a local directory that needs mounting), and executes the final GraphQL query.
...
- **Zero-Config Contexts** — Pass high-level objects (`Directory`, `Container`, `File`) as arguments directly. The interpreter detects them and resolves their GraphQL IDs automatically via `GDeferred` nodes.
- **Type Safety** — Full Gleam types for enums, inputs, and scalars. No more guessing GraphQL field names.
- **Idiomatic Gleam** — Leverages `use` syntax and CPS for a readable, functional flow with no hidden effects.
- **DSL Code Generation** — Fully automated from Dagger's GraphQL introspection schema.
...
```
core/                   # dagger_core — engine
  src/dagger/
    types.gleam         # operation tree + GraphQL AST
    interpreter.gleam   # eval/apply logic, recursive ID resolution
    client.gleam        # HTTP transport

codegen/                # dagger_codegen — code generator
  src/codegen/
    printer.gleam       # schema → Gleam source

sdk/                    # dagger_gleam — the SDK
  src/
    dagger.gleam        # public entrypoint (connect, Client)
    dagger/dsl/         # auto-generated by dgl generate
    examples/           # cookbook — runnable examples
      basic_pipeline.gleam
  test/
    dagger/
      dagger_gleam_test.gleam
```
